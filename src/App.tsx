...
import { ArrowTrendingUpIcon, Bars3Icon, FireIcon, StarIcon, XMarkIcon, HomeIcon } from '@heroicons/react/24/outline';
...
const App = () => {
  ...
  const [navigation, setNavigation] = useState([
    ...
    { name: 'Top Rated', href: '#', icon: FireIcon, current: false },
    { name: 'Trending', href: '#', icon: ArrowTrendingUpIcon, current: false },
    { name: 'Favorites', href: '#', icon: StarIcon, current: false },
  ]);
  console.log('navigation:', navigation);

  const updateNavigation = (name: string) => {
    setNavigation(navigation.map((item) => ({ ...item, current: item.name === name })));
  }

  return (
    <>
      <div>
        {/* Mobile sidebar, show/hide based on sidebarOpen state. */}
        <Dialog open={sidebarOpen} onClose={setSidebarOpen} className="relative z-50 lg:hidden">
          ...
          <div className="fixed inset-0 flex">
            ...
            >
              ...
              <div
                className="relative flex grow flex-col gap-y-5 overflow-y-auto bg-gray-900 px-6 pb-2 ring-1 ring-white/10">
                <button className="relative flex h-16 shrink-0 items-center" onClick={() => updateNavigation('Home')}>
                  <img
                    alt="Your Company"
        ...
        <div className="hidden bg-gray-900 lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
          ...
          <div className="flex grow flex-col gap-y-5 overflow-y-auto border-r border-gray-200 px-6">
            <button className="flex h-16 shrink-0 items-center" onClick={() => updateNavigation('Home')}>
              <img
                alt="Your Company"
                src="https://tailwindcss.com/plus-assets/img/logos/mark.svg?color=indigo&shade=600"
                className="h-8 w-auto"
        ...
        <main className="py-10 lg:pl-72">
          <div className="px-4 sm:px-6 lg:px-8">
            {/* Your content */}
            {navigation[0].current && <Landing />}
            {navigation[1].current && <TopRated />}
            {navigation[2].current && <Trending />}
            {navigation[3].current && <Favorites />}
          </div>
        </main>
      </div>
    </>
